...
app.post('/users/:userId/query', auth, async (req, res) => {
    ...
    try {
        ...
        const systemPrompt = {
            role: "system",
            content: "You are a helpful assistant that analyzes financial transactions and provides insights."
        };

        // User's query about their transactions
        const userPrompt = {
            role: "user",
            content: `I have the following transactions: ${JSON.stringify(transactionsData)}. Based on this data, ${query}`
        };

        // Making the request to OpenAI
        const completion = await openai.chat.completions.create({
            model: "gpt-3.5-turbo", // Ensure you use the correct model ID that you have access to
