...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ id: user.rows[0].user_id }, JWT_SECRET, { expiresIn: '1h' });
...
app.get('/transactions/:transactionId', auth, async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT * FROM transactions WHERE transaction_id = $1 AND user_id = $2', [transactionId, req.user]);
...
app.delete('/transactions/:transactionId', auth, async (req, res) => {
    const { transactionId } = req.params;
    try {
        const result = await pool.query('DELETE FROM transactions WHERE transaction_id = $1 AND user_id = $2 RETURNING *', [transactionId, req.user]);
        if (result.rows.length > 0) {
            res.status(204).send();
        } else {
            res.status(404).json({ msg: 'Transaction not found' });
